// Background story processor, meant to be invoked on a schedule (e.g. every
// minute) by Lovable's cron scheduler. This is what lets story generation
// keep going even if the person closes the tab: whichever of the open-tab
// loop (see story.$storyId.tsx) or this sweep gets to a story first does the
// next step, and retries/failure bookkeeping (see recordAttemptFailure in
// story.functions.ts) is shared between both paths.
